...
"  -log\n"
"  -harris <real:sigma>\n"
"  -featureDetection <real:sigma>\n"
"  -scaledFeatureDetection\n"
...
"  -sharpen \n"
"  -median \n"
"  -bilateral <real:sigmaS, real:sigmaR> \n"
...
main(int argc, char **argv)
{
  ...
  while (argc > 0) {
    ...
    else if (!strcmp(*argv, "-featureDetection")) {
        CheckOption(*argv, argc, 2);
        double sigma = atof(argv[1]);
        argv += 2, argc -= 2;
        image->FeatureDetection(sigma);
      }
      else if (!strcmp(*argv, "-scaledFeatureDetection")) {
        argv++, argc--;
        image->ScaledFeatureDetection();
      }
    ...
    else if (!strcmp(*argv, "-lensDistortion")) {
      argv++, argc--;
      image->LensDistortion();
    }
    else if (!strcmp(*argv, "-highPassSharpen")) {
      CheckOption(*argv, argc, 2);
      double sigma = atof(argv[1]);
      argv += 2, argc -= 2;
      image->HighPassSharpen(sigma);
    }
    else if (!strcmp(*argv, "-median")) {
      CheckOption(*argv, argc, 2);
      int radius = atof(argv[1]);
      argv += 2, argc -= 2;
      image->Median(radius);
    }
    else if (!strcmp(*argv, "-bilateral")) {
      CheckOption(*argv, argc, 3);
      double sigmaS = atof(argv[1]);
      double sigmaR = atof(argv[2]);
      argv += 3, argc -= 3;
      image->Bilateral(sigmaS, sigmaR);
    }
