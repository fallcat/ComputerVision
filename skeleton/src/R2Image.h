...
class R2Image {
 public:
  ...
  void FeatureDetection(double sigma);
  void ScaledFeatureDetection();
  void Sharpen(void);
  ...
  void Bilateral(double sigmaS, double sigmaR);
 ...
 private:
  ...
  struct SearchSSD
  {
      ...
      SearchSSD(int x, int y, double ssd)
      {
          searchX = x;
          searchY = y;
          searchSSD = ssd;
      ...
      bool operator<(const SearchSSD& search) const {
        return searchSSD < search.searchSSD;
      }
  ...
  struct ScaledFeature
  {
      ...
      ScaledFeature(int x, int y, R2Pixel val, double s)
      {
          centerX = x;
          centerY = y;
          HarrisValue = val;
